...
public class InscriptionServiceImpl implements InscriptionService {
    ...
    @Override
    @Transactional
    public void supprimerMembreDuClub(UUID membreId, UUID clubId) {
        // Trouver l'inscription du membre dans ce club spécifique
        Optional<Inscription> inscriptionOpt = inscriptionRepository.findByUtilisateurEmailAndClubId(
        ...
        if (inscriptionOpt.isPresent()) {
            // Supprimer l'inscription (ce qui retire le membre du club)
            inscriptionRepository.delete(inscriptionOpt.get());
        } else {
            throw new RuntimeException("Ce membre n'est pas inscrit dans ce club");
        }
    }
}